...
y_data = pd.read_excel("Outputs-Unsaturated concentration.xlsx")
x_c = x_data.columns
...
def train_model(model, model_name, file_name, val_data, val_labels):
    ...
    joblib.dump(model, r"D:\博士期间所有文件汇总\十一老师-千城\集成学习-热解油可解释性\12.5-集成学习-V1\真实开始\Unsaturated concentration\model\{}.pkl".format(model_name))
...
def on_step(optim_result):
    # 每次迭代后保存 RMSE
    results.append(optim_result['fun'] * 10)
    print(f"Iteration {len(results)} - RMSE: {optim_result['fun'] * 10}")
